function [ind,cost,costmat]=sco_match_vector(z1,z2,varargin)
default={'direction','columns','costfunction',@(x,y)norm(x-y,2),...
    'warningcost',Inf,'costmatonly',false};
options=sco_set_options(default,varargin,'pass_on');
if strcmp(options.direction,'rows')
    z1=z1';
    z2=z2';
end
n1=size(z1,2);
...
for i=1:n1
    for j=1:n2
        costmat(i,j)=options.costfunction(z1(:,i),z2(:,j));
    end
end
if options.costmatonly
    ind=costmat;
    cost=costmat;
    return
end
ind=munkres(costmat);
cost=diag(costmat(:,ind))';
scost=sum(cost);
if scost>options.warningcost
    warning('sco_match_vector:poor_match',...
        'sco_match_vector: sum of assignment cost %g exceeded warninglevel %g',...
        scost,options.warningcost);
end
end
