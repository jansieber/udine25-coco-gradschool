%% Basic demo for use of symbolic derivatives  using coco demo bistable
% The right-hand side for the ODE (the Duffing
...
%
% Outputs are |F|, function handle (output of sco_gen) that returns
% functions and derivatives, |funcstr|, the text of the function body and
% |derivatives|, the symbolic expressions for all directional derivatives
F=sco_sym2funcs(f,...             % symbolic expression for f
    ...
    'filename','bistable',... % filename for result
    'maxorder',3);              % derivatives are computed up to this order (default 2)
...
baseargs={t0',x0',p0};
f_t=f(baseargs{:});              % values of f(t,x(t),p) along orbit
dfx_t=dfx(baseargs{:});           % values of df/dx(t,x(t),p) along orbit
...
%% Test total, mixed and high derivatives
% Total derivatives can be created by calling F with just a number (the
% degree of the derivative). Derivatives of order higher than two are
% not currently used in the demo, but are useful for normal form
% coefficient monitoring. The examples below create
% 
...
% 
df3=F(3); % third-order total or directional derivative
dfxpdir=@(t,x,p,dt,dx,dp)feval(F({3}),t,x,p,{0,0,dt},{'I',0,dx},{0,'I',dp})
dev={1,0*x0'+1,0*p0+2};
dfxpdirval=dfxpdir(baseargs{:},dev{:});
size(dfxpdirval)
dfxxvp=F({'x','x*v','p'});
xdev=0*x0'+1;
dfxxpval=dfxxvp(baseargs{:},xdev)
size(dfxxpval)
