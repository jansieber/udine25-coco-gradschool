function [u0,udev,udims,argdim]=arg_expand_directional(fmt,varargin)
%% expand those direction that have not
...
if fmt.debug
    assert(numel(devargs)==fmt.nargs,...
        'sco:args',...
        'sco: number of arguments given, %d, different from expected: %d',...
        numel(devargs),fmt.nargs);
end
