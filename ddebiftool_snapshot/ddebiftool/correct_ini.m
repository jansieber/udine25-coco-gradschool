...
tangent_dim=length(dir);
pnull=repmat(pini,0,1);
if correc
    %% correct initial guess
    free_par=parameter_fix(branch.parameter,pini);
    if tangent_dim>0
        pnull=p_tangent(funcs,mth,pini,free_par,'tangent_dim',tangent_dim);
    end
...
if ~isempty(dir)
    ...
    if tangent_dim>0
        pnull=p_tangent(funcs,mth,p2,free_par,'tangent_dim',tangent_dim);
    end
...
pnew=dde_point_from_x(a*x1+x2,p2,free_par);
end
