function varargout=dde_hcli_eigval_jac_res(funcs,pt,free_par,method,varargin)
%% Jacobian and residual for connecting orbits
% function [J,res]=dde_subspace_hcli_jac_res(funcs,pt,free_par,method,pref,varargin)
% INPUT:
...
default={'pref',pt,'collocation_parameters',method.collocation_parameters,...
    'phase_condition',method.phase_condition,'output','J'};
...
rhs=funcs.wrap_rhs;
sys_deri=funcs.wrap_deri_old;
%drhs_dir=funcs.drhs_dir;
%drhs_mf=funcs.drhs_mf;
...
tp_del=funcs.tp_del;
if tp_del==0
    ind_tau=sys_tau(); % delay numbers
    tau=par(ind_tau); % delay values
    ntau=length(ind_tau); % number of delays
else
...
npar=length(free_par); % number of free parameters
...
J=zeros(nml+3*n+(s1+s2)*n+s1+2*s2+1+1,nml_n_1+2*n+(s1+s2)*n+2*s1+s2+npar);
...
% express former poly_elg (Lagrange polynomial evaluation using
% dde_coll_eva
loc_elg=@(m,t)dde_coll_eva(eye(m+1),(0:m)/m,t(:)',m)';
loc_del=@(m,t)dde_coll_eva(eye(m+1),(0:m)/m,t(:)',m,'diff',1)';
loc_lgr=@(msh,t)dde_coll_eva(eye(m+1),msh,t(:)',m);
loc_dla=@(msh,t)dde_coll_eva(eye(m+1),msh,t(:)',m,'diff',1);
% more initialisation:

all_dPa=loc_dla((0:m)/m,c(:)')';
all_Pa=loc_lgr((0:m)/m,c(:)')';
    ...
    for m_i=1:m
        ...
        for tau_i=1:ntau
            ...
            if (~initial_function_segment(tau_i))
                ...
                Pb(tau_i,:)=loc_elg(m,c_tau_trans(tau_i));
        ...
        for t_i=1:ntau
            ...
            if (~initial_function_segment(t_i))
                ...
                dPb=loc_del(m,c_tau_trans(t_i))/hhh_tau(t_i);
                ...
                for k=1:s1
                    J(i_range,nml_n_1+npar+2*n+s1*(n+1)+s2*(n+1)+k)=...
                        J(i_range,nml_n_1+npar+2*n+s1*(n+1)+s2*(n+1)+k)-...
                ...
                J(i_range,nml_n_1+npar+1:nml_n_1+npar+n)=...
                    J(i_range,nml_n_1+npar+1:nml_n_1+npar+n)-T_A1;
                
                % derivatives to v(k)
                
                for k=1:s1
                    J(i_range,nml_n_1+npar+2*n+(k-1)*n+1:nml_n_1+npar+2*n+k*n)=...
                        J(i_range,nml_n_1+npar+2*n+(k-1)*n+1:nml_n_1+npar+2*n+k*n)-...
                ...
                for k=1:s1
                    J(i_range,nml_n_1+npar+2*n+s1*n+k)=...
                        J(i_range,nml_n_1+npar+2*n+s1*n+k)...
...
xx1=x1(:,ones(ntau+1,1));

res(nml+1:nml+n,1)=rhs(xx1,par);
ieq.stst1=nml+1:nml+n;
% derivatives to x(t-tau(i))

for i=0:ntau
    J(nml+1:nml+n,nml_n_1+npar+1:nml_n_1+npar+n)=...
        J(nml+1:nml+n,nml_n_1+npar+1:nml_n_1+npar+n)...
        +sys_deri(xx1,par,i,[],[]);
end
...
xx2=x2(:,ones(ntau+1,1));

res(nml_n_1:nml+2*n)=rhs(xx2,par);
ieq.stst2=nml_n_1:nml+2*n;
for i=0:ntau
    J(nml_n_1:nml+2*n,nml_n_1+npar+n+1:nml_n_1+npar+2*n)=...
        J(nml_n_1:nml+2*n,nml_n_1+npar+n+1:nml_n_1+npar+2*n)...
        +sys_deri(xx2,par,i,[],[]);
end
...
ieq.veq=nml+2*n+1:nml+2*n+s1*n;
% entries for the v_k variables

for k=1:s1
    J(nml+2*n+(k-1)*n+1:nml+(k+2)*n,...
        nml_n_1+npar+2*n+(k-1)*n+1:nml_n_1+npar+2*n+k*n)=...
...
for k=1:s1
    J(nml+2*n+(k-1)*n+1:nml+(k+2)*n,nml_n_1+npar+2*n+s1*n+k)=...
        J(nml+2*n+(k-1)*n+1:nml+(k+2)*n,nml_n_1+npar+2*n+s1*n+k)+v(:,k);
    for i=1:ntau
        J(nml+2*n+(k-1)*n+1:nml+(k+2)*n,nml_n_1+npar+2*n+s1*n+k)=...
            J(nml+2*n+(k-1)*n+1:nml+(k+2)*n,nml_n_1+npar+2*n+s1*n+k)+...
...
for k=1:s1
    for i=0:ntau
        J(nml+2*n+(k-1)*n+1:nml+(2+k)*n,nml_n_1+npar+1:nml_n_1+npar+n)=...
            J(nml+2*n+(k-1)*n+1:nml+(2+k)*n,nml_n_1+npar+1:nml_n_1+npar+n)-...
...
for k=1:s1
    ...
    J(nml+2*n+s1*n+k,nml_n_1+npar+2*n+(k-1)*n+1:nml_n_1+npar+2*n+k*n)=...
        2*v(:,k)';
end
ieq.vnorm=nml+2*n+s1*n+(1:s1);
...
ieq.weq=nml+2*n+s1*(n+1)+1:nml+2*n+s1*n:nml+2*n+s1*(n+1)+s2*n;

% entries for the w_k variables

for k=1:s2
    J(nml+2*n+s1*(n+1)+(k-1)*n+1:nml+2*n+s1*(n+1)+k*n,...
        nml_n_1+npar+2*n+s1*(n+1)+(k-1)*n+1:nml_n_1+npar+2*n+s1*(n+1)+k*n)=...
        J(nml+2*n+s1*(n+1)+(k-1)*n+1:nml+2*n+s1*(n+1)+k*n,...
        nml_n_1+npar+2*n+s1*(n+1)+(k-1)*n+1:...
        nml_n_1+npar+2*n+s1*(n+1)+k*n)+...
...
for k=1:s2
    J(nml+2*n+s1*(n+1)+(k-1)*n+1:...
        nml+2*n+s1*(n+1)+k*n,nml_n_1+npar+2*n+s1*(n+1)+s2*n+k)=...
        J(nml+2*n+s1*(n+1)+(k-1)*n+1:...
        nml+(k+2)*n+s1*(n+1),nml_n_1+npar+2*n+s1*(n+1)+s2*n+k)+w(:,k);
    for j=1:ntau
        J(nml+2*n+s1*(n+1)+(k-1)*n+1:...
            nml+(k+2)*n+s1*(n+1),nml_n_1+npar+2*n+s1*(n+1)+s2*n+k)=...
            J(nml+2*n+s1*(n+1)+(k-1)*n+1:...
            nml+(k+2)*n+s1*(n+1),nml_n_1+npar+2*n+s1*(n+1)+s2*n+k)+...
...
for k=1:s2
    for i=0:ntau
        for j=1:n
            J(nml+2*n+(k-1)*n+s1*(n+1)+j,nml_n_1+npar+n+(1:n))=...
                J(nml+2*n+(k-1)*n+s1*(n+1)+j,nml_n_1+npar+n+(1:n))-...
                w(:,k)'*sys_deri(xx2,par,[i 0],[],In(:,j));
        end
        for j=1:ntau
            for l=1:n
                J(nml+2*n+(k-1)*n+s1*(n+1)+l,nml_n_1+npar+n+(1:n))=...
                    J(nml+2*n+(k-1)*n+s1*(n+1)+l,nml_n_1+npar+n+(1:n))-...
...
ieq.wnorm=nml+2*n+s1*(n+1)+s2*n+(1:s2);
% integral orthogonality conditions

for k=1:s2
    res(nml+2*n+s1*n+s1+s2*n+s2+k,1)=w(:,k)'*(profile(:,ml+1)-x2);
end
ieq.intorth=nml+2*n+s1*n+s1+s2*n+s2+(1:s2);

% first term of derivative to w_k
for k=1:s2
    J(nml+2*n+s1*n+s1+s2*n+s2+k,...
        nml_n_1+npar+2*n+s1*n+s1+(k-1)*n+1:nml_n_1+npar+2*n+s1*n+s1+k*n)=...
        J(nml+2*n+s1*n+s1+s2*n+s2+k,...
        nml_n_1+npar+2*n+s1*n+s1+(k-1)*n+1:...
        nml_n_1+npar+2*n+s1*n+s1+k*n)+...
        (profile(:,ml+1)-x2)';
end
...
for l=1:ntau
    if (1-tau(l)/T)~=1  % complicated here because of machine precision
        ...
        % recall that gauss_c=legpts(m,[0,1]);
        ...
        for i=1:m
            Pa(index-K+i,:)=...
                loc_elg(m,(gauss_c_trans(index-K+i)-tau_T)/(t(index+m)-tau_T));
        ...
        for index=K+m:m:ml-m+1
            ...
            for i=1:m
                Pa(index-K+i,:)=loc_elg(m,(gauss_c_trans(index-K+i)-...
        ...
        for k=1:s2
            for i=1:length(gauss_weight)
                J(nml+2*n+s1*n+s1+s2*n+s2+k,nml_n_1+npar+2*n+s1*n+s1+s2*n+k)=...
                    J(nml+2*n+s1*n+s1+s2*n+s2+k,nml_n_1+npar+2*n+s1*n+s1+s2*n+k)-...
        ...
        for k=1:s2
            J(nml+2*n+s1*n+s1+s2*n+s2+k,nml_n_1+npar+n+1:...
                nml_n_1+npar+2*n)=-w(:,k)';
            for i=1:length(gauss_weight)
                dA1_dx2= sys_deri(xx2,par,[l 0],[],gauss_y(:,i));
                for j=1:ntau
                    dA1_dx2= dA1_dx2+sys_deri(xx2,par,[l j],[],gauss_y(:,i));
                end
                J(nml+2*n+s1*n+s1+s2*n+s2+k,nml_n_1+npar+n+1:nml_n_1+npar+2*n)=...
                    J(nml+2*n+s1*n+s1+s2*n+s2+k,nml_n_1+npar+n+1:nml_n_1+npar+2*n)+...
...
ieq.projection=nml+2*n+s1*(n+1)+s2*n+s2+(1:s2);
...
ieq.continuity=nml+2*n+s1*n+s1+s2*n+2*s2+(1:n);

% derivative to x1

J(nml+2*n+s1*(n+1)+s2*(n+2)+1:nml+2*n+s1*(n+1)+s2*(n+2)+n,...
    nml_n_1+npar+1:nml_n_1+npar+n)=-eye(n);

% derivative to alpha(k)

J(nml+2*n+s1*(n+1)+s2*(n+2)+1:nml+2*n+s1*(n+1)+s2*(n+2)+n,...
    nml_n_1+npar+2*n+s1*n+s1+s2*n+s2+1:...
    nml_n_1+npar+2*n+s1*n+s1+s2*n+s2+s1)=-epsilon*v;
...
if ph && non_gauss
    for l_i=1:l
        ...
        for k=1:m
            ...
            dPa=loc_dla(t(index_a:index_a+m),gauss_c(k));
            Pa=loc_lgr(t(index_a:index_a+m),gauss_c(k));
            u_prime_previous=previous.profile(:,index_a:index_a+m)*dPa';
            for q=1:m+1
...
ieq.ph=nml+2*n+s1*(n+1)+s2*(n+2)+n+1;
...
ieq.alphanorm=nml+2*n+s1*n+s1+s2*n+s2+s2+n+2;
J(nml+2*n+s1*n+s1+s2*n+s2+s2+n+2,...
    nml_n_1+npar+2*n+s1*(n+1)+s2*(n+1)+1:...
    nml_n_1+npar+2*n+s1*(n+1)+s2*(n+1)+s1)=2*alpha';
