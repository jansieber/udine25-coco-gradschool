...
% $Id$
%%
default={'funcs',[],'stst_correct',true,'method',[],'free_par',[],'evtype','l0',...
    'evtol',-1e-5};
...
stab=stst(1).stability.(options.evtype);
assert(...
    ~isempty(stab) &&...
    max(real(stab))>=options.evtol,...
...
lambda_w=reshape(stst(2).stability.(options.evtype),[],1);
w_sel=real(lambda_w)>0 & imag(lambda_w)>=0;
lambda_w=lambda_w(w_sel);
hcli.lambda_w=lambda_w;
hcli.w=conj(stst(2).stability.w(:,w_sel));
for i=1:size(hcli.w,2)
    hcli.w(:,i)=hcli.w(:,i)./sqrt(hcli.w(:,i)'*hcli.w(:,i));
end
for i=1:size(hcli.v,2)
    hcli.v(:,i)=hcli.v(:,i)./sqrt(hcli.v(:,i)'*hcli.v(:,i));
end
