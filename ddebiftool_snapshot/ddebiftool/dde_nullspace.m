...
if ~issparse(A)
    ...
    if ~isempty(options.nulldim)
        ...
        sv=diag(S);
        nulldim=size(A,2)-sum(sv>options.nulltol);
    end
    nulldim=min(size(V,2),nulldim);
    v=V(:,end-nulldim+1:end);
    return
end
if s1<s2
...
[v,dum]=qr(full(v),0); %#ok<ASGLU>
if options.check
    res=max(abs(A*v),[],1);
    An=norm(A,'inf');
    errlarge=find(res(:)>options.nulltol*An);
    if ~isempty(errlarge)
        [dum,ix]=max(abs(res(errlarge))); %#ok<ASGLU>
