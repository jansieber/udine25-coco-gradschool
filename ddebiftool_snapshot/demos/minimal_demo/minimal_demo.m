...
%
% <html>
% $Id: minimal_demo.m 369 2019-08-27 00:07:02Z jansieber $
% </html>
%
...
rhs=@(xx,p)[...
     xx(2,1,:);...
    -p(indp.d)*xx(2,1,:)-p(indp.a)*xx(1,1,:)-xx(1,1,:).^3-p(indp.b)*(xx(1,1,:)-xx(1,2,:))];
%% Call to set_funcs(...)
...
funcs=set_funcs('sys_rhs',rhs,'sys_tau',@()indp.tau,'x_vectorized',true); %#ok<NASGU>
...
% above.
funcs=set_symfuncs(@sym_minimal_demo,'sys_tau',@()indp.tau);
...
br_args={'max_bound',[indp.tau,20;indp.b,0.6],...
    'min_bound',[indp.tau 0; indp.b,0],...
    'max_step',[0,2;indp.tau,0.2;indp.b,0.01]};
...
% 
%#ok<*NOPTS>
par([indp.tau,indp.a,indp.b,indp.d])=...
        [0,    0.5,  0.6, 0.05];
[triv_eqs,suc]=SetupStst(funcs,'x',[0;0],'parameter',par,...
    'contpar',indp.tau,br_args{:},'minimal_real_part',-1)
...
[triv_eqs,nunst_eqs]=br_stabl(funcs,triv_eqs);
...
% branches).
[triv_eqs,~,ind_hopf,stst_bifs]=LocateSpecialPoints(funcs,triv_eqs,'debug',true) 
...
[per_orb,suc]=SetupPsol(funcs,triv_eqs,ind_hopf(2),'intervals',30,'degree',4,...
    'max_bound',[indp.tau,20],'max_step',[0,4;indp.tau,0.5]);
figure(1);hold(ax1,'on');
per_orb.method.continuation.stops={@(p)norm(p(end).profile,'inf')<1e-5};
per_orb=br_contn(funcs,per_orb,300,'plotaxis',ax1);
[per_orb,nunst_per,dom_per,err_per]=br_stabl(funcs,per_orb);
...
Plot2dBranch({triv_eqs,per_orb},'ax',ax3);
grid(ax3,'on');
xlabel(ax3,'tau');
ylabel(ax3,'max(x)');
...
hopfopts={'step',-1e-3,'max_step',[indp.tau,0.2;indp.b,0.01],'max_bound',[indp.b,0.6]};
hopf=SetupHopf(funcs,triv_eqs,ind_hopf(2),'contpar',[indp.b,indp.tau],'dir',indp.b,...
    hopfopts{:});
figure(1);clf;ax1=gca;
nbr2_steps=200;
...
% the first 3 points along each branch.  See
...
for i=length(C1branches):-1:1
    [C1branches(i),nunst_br{i},dom_ev{i},triv_err{i}]=...
        br_stabl(C1info(i).funcs,C1branches(i));
    C1branches(i)=LocateSpecialPoints(C1info(i).funcs,C1branches(i));
end
%% plot all branches
figure(2);
for i=1:length(C1branches)
    Plot2dBranch(C1branches(i),'ax',ax2,'funcs',C1info(i).funcs);
end
