...
% results.
%% Convergence rates
% check error estimates for first nev=10 eigenvalues for increasing number
% of chebyshev nodes for $x'(t)=-x(t-\pi/2)$. Range of chebyshev nodes:
% ncheb=nev*2 increasing for as long as the error estimate decreases
dim=1;
A=cat(3,0,-1); % x'(t)=-x(t-tau(1))
tau=pi/2;
nev=10;
ncheb=ceil(nev*2);
err=zeros(nev,1);
...
ncheblist=ncheb;
i=0;
while maxerr(1)>maxerr(2) || maxerr(2)>errmax
    i=i+1;
    ncheblist(i)=ncheb;
    stability=dde_stst_eig_cheb(A,tau,...
    'max_number_of_eigenvalues',nev,'minimal_real_part',-Inf,...
    'maxsize',dim*ncheb,'inisize',ncheb,'ncheb',ncheb,'root_accuracy',1e10);
    err(:,i)=stability.err(1:nev);
    [maxerr(1),maxerr(2)]=deal(maxerr(2),max(err(:,i)));
    lam(:,i)=stability.l0(1:nev);
    ncheb=ncheb+1;
end
err_rel=err./repmat(reshape(abs(stability.l0(1:nev)),[],1),1,size(err,2));
figure(4);
pl=semilogy(ncheblist,err_rel,'o-','LineWidth',2);
grid on
set(gca,'LineWidth',1,'GridLineWidth',1,...
    'FontSize',15,'FontName','Courier','FontWeight','bold');
