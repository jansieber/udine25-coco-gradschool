...
%
% <html>
% $Id: minimal_coco_demo.m 353 2019-06-30 23:06:52Z jansieber $
% </html>
%
...
rhs=@(xx,p)[...
     xx(2,1,:);...
    -p(ip.d)*xx(2,1,:)-p(ip.a)*xx(1,1,:)-xx(1,1,:).^3-p(ip.b)*(xx(1,1,:)-xx(1,2,:))];
%% Call to set_funcs(...)
...
% function.
%funcs=set_funcs('sys_rhs',rhs,'sys_tau',@()ip.tau,'x_vectorized',true); %#ok<NASGU>
...
% in <../../minimal_demo/html/gen_sym_minimal_demo.html> The wrapper |set_symfuncs| converts this
...
% above.
funcs=set_symfuncs(@sym_minimal_demo,'sys_tau',@()ip.tau);
...
prob=coco_prob();
prob=dde_construct(prob,'stst','funcs',funcs,'info',pt,'pnames',ip);
...
% bifurcation. The constructor for the emanating torus bifurcations is not
% yet implemented for coco as this requires normal form monitoring.
% (|C1branch_from_C2point| is the DDE-Biftool routine.)
...
%% Plot torusbifs
axes(ax2);
nruntr=length(coco_bd_labs(coco_bd_read('runpsol'),'TR'));
for i=nruntr:-1:1
    rname=sprintf('runtr%d',i);
...
dde_plot_legend(ax2,'location','best');
set(ax2,lw{:},txt{:});
set(ax1,lw{:},txt{:});
