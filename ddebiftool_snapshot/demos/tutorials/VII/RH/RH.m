...
ntrsteps=1000; [nsbr,suc]=br_contn(trfuncs,nsbr,ntrsteps); assert(suc>0)
...
[~,fbr,suc]=C1branch_from_C2point(funcs,zeho,unfolding_pars,...
    ...
    'step',1e-03,'plot',0);
assert(all(suc(:)>0))
nop=1000; [fbr,suc]=br_contn(funcs,fbr,nop); assert(suc>0)
fbr=br_rvers(fbr);
[fbr,suc]=br_contn(funcs,fbr,nop); assert(suc>0)
%% Detect special points on the Hopf branch
[hbr_wbifs,hopftests,~,~]=LocateSpecialPoints(funcs,hbr);
...
L1s=hopftests.genh(1,:);
hbrsub.point=hbr_wbifs.point(L1s>0);
hbrsup.point=hbr_wbifs.point(L1s<0);
hbrsub_pm   = [getpars(hbrsub,ind.Iapp); getpars(hbrsub,ind.S)];
