%% Coupled FHN neural system with delay (Using Finite Differenences)
% 
...
FHN_sys = @(xx,par) [...
    -xx(1,1,:).^3/3+(c+par(1,ind.alpha,:)).*xx(1,1,:).^2+d*xx(1,1,:)-xx(2,1,:)+2*par(1,ind.beta,:).*tanh(xx(1,2,:));
     epsilon*(xx(1,1,:)-b*xx(2,1,:))];
%% Set funcs structure
funcs=set_funcs('sys_rhs',FHN_sys,'sys_tau',@()ind.tau,'x_vectorized',true,'p_vectorized',true);
