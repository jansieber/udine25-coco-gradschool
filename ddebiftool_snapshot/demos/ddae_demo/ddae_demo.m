...
%
% $Id: ddae_demo.m 374 2019-09-14 14:02:58Z jansieber $
%%
base=[pwd(),'/../../'];
...
rfuncs=set_rotfuncs('wrap_rhs',dde_sym2fun(@sym_ddae_demo,'rhs'),...
    'wrap_dirderi',dde_sym2fun(@sym_ddae_demo,'rhs_dirderi'),...
...
[foldfuncs,rw_fold,suc]=SetupRWFold(rfuncs,rw_branch,ind_fold,'contpar',[ip.b,ip.a,ip.omega],...
    'dir',ip.b,'newton_max_iterations',6);
figure(2);ax2=gca;
...
[mw_branch,suc]=SetupPsol(rfuncs, rw_branch,ind_hopf,...
    'intervals',50,'degree',degree,...
    opt_inputs{:},'matrix','sparse','radius',0.1,'max_step',[0,0.1],...
    'max_bound',[ip.b,1.01],'excludefreqs',0)
figure(1);
...
% We initialize a POfold branch computation from the torus bifurcation in
% the point where a Floquet multiplier moves through 1.
ind_pofold=find(tr_nunst==1,1,'first')-1;
[pofoldfuncs,pofold_branch,suc]=SetupMWFold(trfuncs,tr_branch,ind_pofold,'contpar',[ip.b,ip.a,ip.omega],...
    'dir',ip.b,'print_residual_info',1,'max_bound',[ip.b,1.01;ip.a,10],...
    'max_step',[0,0.2])
