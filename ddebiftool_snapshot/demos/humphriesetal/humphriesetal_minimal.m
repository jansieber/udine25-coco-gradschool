...
% for periodic-orbit bifurcation tracking with state-dependent delay,
% degree higher than 3 is safer since derivatives w.r.t time enter r.h.s.
% and 2nd derivatives enter Jacobian.
t_args={'print_residual_info',1,...
    'stop_1_2',true,'use_tangent',true,'minimal_angle',0,'degree',5,'intervals',31};
nbr2_steps=200;
C1info=BranchFromCodimension2(funcs,hbranch0,t_args{:},brbounds{:},'step',1e-2);
...
for i=length(C1info):-1:1
    ...
    switch C1info(i).C1type
        case {'hopf','fold'}
            locate_special=@LocateSpecialPoints;
            C1info(i).branch.method.point.print_residual_info=0;
        case 'TorusBifurcation'
            locate_special=@(f,b)MonitorChange(f,b,'printlevel',2,'print_residual_info',0);
    end
    C1branches(i)=br_contn(C1info(i).funcs,C1info(i).branch,nbr2_steps,'plotaxis',ax3,...
        'print_progress',1,t_args{:});
    if C1info(i).reverse
        fprintf('Reversing %s\n',[C1info(i).C1type]);
    ...
    [C1branches(i),testfuncs{i},indc2{i},ind2ctype{i}]=...
        locate_special(C1info(i).funcs,C1branches(i));
...
%% The first torus bifurcation crosses folds, track the first one
...
[pffuncs,pfbr,suc]=SetupPOfold(C1info(1).funcs,C1branches(1),ind_pfold,...
    'contpar',[ip.kappa1,ip.kappa2],'dir',ip.kappa1,'step',0.1,brbounds{:})
figure(3);
pfbr=br_contn(pffuncs,pfbr,50,'plotaxis',ax3);
pfbr=br_rvers(pfbr);
pfbr=br_contn(pffuncs,pfbr,50,'plotaxis',ax3);
