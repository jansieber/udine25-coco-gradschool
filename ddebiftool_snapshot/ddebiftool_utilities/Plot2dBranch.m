...
default={'name',[],{'lgname','DisplayName','displayname'},[],'ax',gca,'funcs',[],'color',[],'linewidth',2,'markersize',8,...
    'bifbdcolor','k','biffillcolor',lines(),'bifbdwidth',1,...
    'stability',0.5,'parameter',branch.parameter.free,'markersymbols','x*sdph+o',...
    'x',[],'y',[],'oldlegend',{[],{}},'pointtype_list',@pointtype_list,...
    'max_nunst',Inf,'excluded_special',{'trivial'}};
...
typenames=fieldnames(getfield(options.pointtype_list(),'codim')); 
if ~isempty(options.name)
    name=options.name;
else
    name=dde_get_kind(pts(1),options.funcs);
end
...
[bd,bifind,labels,markers]=splitbranch(pts,nunst,typenames,options.markersymbols,options.excluded_special);
nparts=size(bd,2);
...
if ~isempty(markers)
    [~,markpos]=ismember(markers,options.markersymbols);
    for i=1:length(bifind)
        lgpt=plot(ax,p1(bifind(i)),p2(bifind(i)),markers(i),...
            'markerfacecolor',options.biffillcolor(markpos(i),:),'markeredgecolor',options.bifbdcolor,...
            pdeco{:},'linewidth',options.bifbdwidth);
...
obj=get(ax,'Children');
sel=arrayfun(@(o)~isempty(get(o,'DisplayName')),obj);
obj=obj(sel);
lgtxt=get(obj,{'DisplayName'});
[lgnames,ix]=unique(lgtxt);
lghandles=obj(ix);
lg={lghandles,lgnames};
legend(ax,lg{1},lg{2},'interpreter','none');
if ~ish
    hold(ax,'off');
end
end
...
%%
function [bd,bifind,labels,markers]=splitbranch(pts,nunst,types,symbols,exclude)
np=length(pts);
if isfield(pts(1),'flag') && ~isempty([pts.flag]) 
    % flags indicate computation of bifurcations occured
    bifind=find(arrayfun(@(x)~isempty(x.flag)&&~ismember(x.flag,exclude),...
        pts));
