...
%
%%
default={'printlevel',0,'distance',branch.method.bifurcation.secant_tolerance,...
...
if ischar(bif_mon_inp)
    bif_monitor=@(p)br_standard_bifurcations(funcs,branch.method,bif_mon_inp,p,pass_on{:});
else
...
while (it<options.min_iterations || dist>options.distance) && absres>0
    ...
    if options.printlevel>0
        fprintf('Bisection: new residual=%g, new distance=%g\n',resnew,dist);
    end
end
