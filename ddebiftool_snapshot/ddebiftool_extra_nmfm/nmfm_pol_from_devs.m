...
% 
% $Id: nmfm_pol_from_devs.m 309 2018-10-28 19:02:42Z jansieber $
%#ok<*AGROW>
%% for test purposes (devs can be real or complex numbers or dev functions)
if isnumeric(devs)
    ax=@(c,dev)sum(c.*dev);
    toconj=@(dev)conj(dev);
    isreal=@(dev)all(imag(dev)==0);
elseif isstruct(devs)
    ax=@nmfm_dev_ax;
    toconj=@nmfm_dev_conj;
    isreal=@(dev)all(imag(dev.lambda)==0)&all(imag(dev.v(:))==0);
elseif iscell(devs)
    ax=@ax_c;
    toconj=@(dev){conj(dev{1})};
    isreal=@(dev)all(imag(dev{1})==0);
end
%% determine polarization sums and factors
[cmats,cfactors]=polarization_coeffs(order,groups);
