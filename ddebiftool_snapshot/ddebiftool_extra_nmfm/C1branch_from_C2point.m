...
%% Initialize branch for continuing codimension 1 branch
% emanating from a codimension 2 point.
%
% @author Maikel Bosschaert, maikel.bosschaert -at- uhasselt.be
% $Id: C1branch_from_C2point.m 376 2022-05-14 15:53:34Z mmbosschaert $
%%
default={'step',1e-2,'steplength_growth_factor',1.1,'codim2','','codim1','','correc',true,'print',1,...
...
if size(options.step,2)==1
    if size(options.step,1)==1
        options.step(1,2)=options.step(1)*options.steplength_growth_factor;
    else
        options.step(:,2)=options.step(:,1)*options.steplength_growth_factor;
    end
end
    ...
    for k=length(br):-1:1
        ...
        for i=1:length(options.step)
            [C1funcs,tmpbranch]=SetupC1funcs(funcs,br(k),i,pass_on{:},...
                'contpar',freepars,'correc',false,'align',pt_align,optargs{:});
            if i==1
                C1branch(k)=tmpbranch;
            else
                C1branch(k).point(i)=tmpbranch.point(1);
            end
            pt_align=tmpbranch.point(1);
        end
    end
else
    C1branch=br;
...
for k=length(C1branch):-1:1
    ...
    if ~options.predictor
        for i=1:length(options.step)
            ...
            secant=p_tangent(C1funcs,C1branch(k).method.point,C1branch(k).point(i),C1branch(k).parameter.free);
            [C1branch(k).point(i),suc(i,k)]=p_correc(C1funcs,C1branch(k).point(i),...
                C1branch(k).parameter.free,secant,C1branch(k).method.point,0,...
                refpoint);
            % [C1branch(k).point(i),suc(i,k)]=p_correc(C1funcs,C1branch(k).point(i),...
