...
%
% $Id: nmfm_pol_real_from_complex.m 309 2018-10-28 19:02:42Z jansieber $
%% 
matcell{1}=[1,0];
factorcell{1}=1;
for j=order-1:-1:1
    g={1:order-j,order-j+1:order};
    [m,f]=polarization_coeffs(order,g);
    matcell{j+1}=m;
    factorcell{j+1}=f;
end
matcell{order+1}=[0,1];
...
[mats,factors]=nmfm_matcollect(cat(1,matcell{:}).',cat(1,factorcell{:}).');
end
