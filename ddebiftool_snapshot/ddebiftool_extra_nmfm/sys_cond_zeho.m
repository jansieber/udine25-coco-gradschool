function [res,J]=sys_cond_zeho(point,pref,get_comp)
...
q1ref=get_comp(pref,'q1');
q1cond=q1ref'*q1-1;
zhres=[q0'*q0-1;real(q1cond);imag(q1cond)];
zhJ=repmat(p_axpy(0,point,[]),3,1);
% derivaties for <q0,q0> -1 and <q1,q1>
zhJ(1).x(ind_q0)=2*q0;
zhJ(2).x(ind_rq1)= real(q1ref);
zhJ(2).x(ind_iq1)=+imag(q1ref);
zhJ(3).x(ind_rq1)=-imag(q1ref);
zhJ(3).x(ind_iq1)= real(q1ref);
