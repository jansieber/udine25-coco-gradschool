...
if ~isempty(options.free_pars)
    ...
    F.J2=@(p1,p2)nmfm_mfderiv(funcs,point,[dp(p1),dp(p2)],pass_on{:});
    F.J3=@(p1,p2,p3)nmfm_mfderiv(funcs,point,[dp(p1),dp(p2),dp(p3)],pass_on{:});
    F.A1=@(v1,v2) nmfm_mfderiv(funcs,point,[v1,dp(v2)],pass_on{:});
    F.A2=@(v1,p1,p2) nmfm_mfderiv(funcs,point,[v1,dp(p1),dp(p2)],pass_on{:});
