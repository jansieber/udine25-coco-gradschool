...
data=fill_pnames(options.pnames,options.parbasename,data);
...
if strcmp(utype,'Stst')
    ...
    if isfield(info,'kind')
        br=df_brnch(funcs,[],info.kind);
        br.point=info;
        br=ChangeBranchParameters(funcs,br,1,varargin{:});
        info=br;
        info.tangent=repmat(info.point,1,0);
    end    
    info_ind={info,1};
...
[data.funcs,data.info]=feval(['Setup',utype],funcs,info_ind{:},...
    'correc',false,'correction',false,varargin{:},'outputfuncs',true);
end
...
%%
function data=fill_pnames(names,base,data)
ind=data.ipar;
pnames=arrayfun(@(i)[base,num2str(i)],ind,'uniformoutput',false);
if isfield(data,'pnames')
    isnamed=cellfun(@(s)~isempty(s),data.pnames);
    pnames(isnamed)=data.pnames(isnamed);
end
if isstruct(names)
...
if isfield(options.info,'point') %info is a branch
    data.ipar=1:length(options.info.point.parameter);
else
    data.ipar=1:length(options.info.parameter);
end
if isstruct(options.pnames)
    data.pnames=fieldnames(options.pnames);
elseif iscell(options.pnames)
    data.pnames=options.pnames;
end    
if iscell(options.extpars)
    ...
    for i=length(options.extpars):-1:1
        options.extpars(i)=find(strcmp(extparnames{i},data.pnames));
    end
end
