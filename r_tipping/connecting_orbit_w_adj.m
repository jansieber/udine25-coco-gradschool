...
frhs={F(''),F('x'),F('par'),F({'x','x'}),F({'x','par'}),F({'par','par'})}; % r.h.s and its partial derivatives wrt x and par
get1=@(x)reshape(x(1,:),[1,size(x,2:ndims(x))]);
bc_prep=@(fname)cellfun(@(f){@(t0,T,x0,x1,p)f(T,x0,x1,p)},...
...
%% Setup BVP for u_-
% b.c.:
bc_um=bc_prep(@r_tipping_bc_minus);
prob=coco_prob();
...
bc_gam=bc_prep(@r_tipping_bc_gamma);
...
bc_up=bc_prep(@r_tipping_bc_plus);
up_pars=um_pars;
Tp0=1e-3;
