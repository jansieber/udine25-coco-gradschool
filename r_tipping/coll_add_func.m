function prob = coll_add_func(prob, sid, fid, varargin)
%
...
if ~isempty(data.dfhan)
  prob = coco_add_func(prob, fid, @coll_func1, @coll_func1_ddu, data, ...
    type, pnames, 'uidx', uidx, 'u0', u0, 'f+df', 'remesh', @coll_remesh);
else
  prob = coco_add_func(prob, fid, @coll_func2, data, ...
    type, pnames, 'uidx', uidx, 'u0', u0, 'remesh', @coll_remesh);
end
end

function [data, y, J] = coll_func1(~, data, u)
xbp = u(data.xbp_idx);
T0  = u(data.T0_idx);
T   = u(data.T_idx);
p   = u(data.p_idx);
...
function [data, J] = coll_func1_ddu(~, data, u)
xbp = u(data.xbp_idx);
T0  = u(data.T0_idx);
T   = u(data.T_idx);
p   = u(data.p_idx);
...
function [data, y] = coll_func2(~, data, u)
xbp = u(data.xbp_idx);
T0  = u(data.T0_idx);
T   = u(data.T_idx);
p   = u(data.p_idx);
...
function [prob, status, xtr] = coll_remesh(prob, data, ~, old_u, old_V) 
